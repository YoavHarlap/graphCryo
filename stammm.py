...
noise_variance = 0  # Set a target noise variance
img_size = 189
img_size = 28
...
def make_for_run(true_labels,n_labeled,n_unlabeled,adj_matrix):
    error_arr = []
    list_unlabeled = list(range(1,4000,50))
    for i in list_unlabeled:
        ...
        curr_error = eror_sum(start, true_labels[n_labeled-i:n_unlabeled+n_labeled])
...
#b = np.ones(51)
#random_labels = np.concatenate((a, b), axis=0)
